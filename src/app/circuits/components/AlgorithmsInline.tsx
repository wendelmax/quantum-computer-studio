...
export default function AlgorithmsInline({ onLoadAlgorithm }: Props) {
  return (
    <div className="rounded-lg p-3 bg-bg-card border border-theme-border transition-all duration-300 hover:border-primary/50">
      <h3 className="text-sm font-medium mb-2 text-theme-text">Algorithms</h3>
      <div className="flex flex-wrap gap-2">
        {(list as Array<{id:string; name:string}>).map(a => (
          <Button
            key={a.id}
            variant="secondary"
            className="px-2 py-1 text-xs transition-all duration-200 hover:scale-105 hover:border-primary/50 active:scale-95"
